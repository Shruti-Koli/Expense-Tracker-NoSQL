...
function showItemOnScreen(obj){
      ...
      displist.innerHTML = displist.innerHTML +`<tr id='${obj._id}'>
          ...
          <td>${obj.descr}</td>
          <td>${obj.category}</td>
          <td><button onclick="deleteExpense('${obj._id}','${obj.descr}','${obj.category}','${obj.price}')">Delete</button></td></tr>`
}

 //delete items from screen
async function deleteExpense(_id,descr,category,price){
    try{
        const token = localStorage.getItem('token');
        console.log(_id);
        let res= await axios.delete("http://localhost:3000/expense/delete-expense/"+_id,{headers:{"auth":token}});
